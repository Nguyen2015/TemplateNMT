...
bash ./scripts/run_gen_template_dept.sh $DATA_DIR false # second parameter [false] indicate the template is parsed in target side
...
cp ../../scripts/iwslt14deen-tgt-dept-step1-setting.sh setting.sh
cd $ROOT_DIR
 
# #  train a template genearator NMT model 
rm $DATA_DIR/checkpoint_last.pt
cp $DATA_DIR/averaged.pt $DATA_DIR/averaged_step1.pt
...
# generate template 
# # --------------
bash ./scripts/translate_template.sh $DATA_DIR "dept"
...
cp ../../scripts/iwslt14deen-tgt-dept-step2-setting.sh setting.sh
cd $ROOT_DIR
...
cp $DATA_DIR/averaged.pt $DATA_DIR/averaged_step2.pt
