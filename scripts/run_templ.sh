...
fairseq-preprocess \
    --source-lang $TEMPLATE_TYPE --target-lang $TGT_LANG \
    --trainpref $PATH_DATA/train --validpref $PATH_DATA/valid --testpref $PATH_DATA/test \
    --destdir $PATH_DATA_BIN --thresholdtgt 0 --thresholdsrc 0 \
    --workers 32 --only-source 
...
CUDA_VISIBLE_DEVICES=$CUDA_VISIBLE_DEVICES fairseq-train \
    ...
    --template-dropout $TEMPL_DROP --template-type $TEMPLATE_TYPE --user-dir $USER_DIR --task template_translation --arch multisources_transformer_iwslt_de_en \
    --update-freq 4 \
    --eval-bleu \
