...
# generate  template from constituent tree 
python src/template_generator.py \
--path_folder_or_pattern "${DATA_DIR}/" \
--type_template syntactic --use_dyn_templ \
...
for TYPE_F in "train" "valid" "test"; do
    mv $DATA_DIR/$TYPE_F.$TEMPL_LANG.softtemplate $DATA_DIR/$TYPE_F.${TEMPL_LANG}_parsed_lent
done